...
import {
  Send, Mic, MicOff, Volume2, VolumeX, Paperclip, Camera,
  ChevronDown, Trash2, Copy, RotateCcw, Loader2
} from 'lucide-react';
import { chatWithSahayak } from '../lib/gemini';
      ...
      <div className={`max-w-[80%] ${isUser ? 'order-first' : ''}`}>
        ...
        {!isUser && (
          <div className="flex items-center gap-2 mt-1 px-1">
            <button onClick={() => onSpeak(message.content)} className="text-gray-400 hover:text-primary transition-colors">
              <Volume2 size={14} />
            </button>
            <button onClick={() => navigator.clipboard.writeText(message.content)} className="text-gray-400 hover:text-primary transition-colors">
              <Copy size={14} />
            </button>
...
export default function Chat() {
  ...
  const sendMessage = async (text = input) => {
    ...
    try {
      ...
      await saveChatMessage(sessionId.current, assistantMsg);
    } catch (err) {
      toast.error(err.message || 'SAHAYAK is offline right now');
      const errMsg = { id: Date.now().toString(), role: 'assistant', content: '⚠️ I\'m having trouble connecting. Please check your internet or API key in Settings.' };
  ...
  const handleVoiceToggle = () => {
    ...
    recognitionRef.current = startVoiceInput(
      ...
      (text, isFinal) => { if (isFinal) { sendMessage(text); setRecording(false); } },
      () => setRecording(false),
      (err) => { toast.error('Voice error: ' + err); setRecording(false); }
    );
  };
  ...
  const clearChat = () => {
    setMessages([{
      id: 'welcome-new',
      role: 'assistant',
      content: 'Chat cleared. नमस्ते! 🙏 What would you like to learn about today?',
    }]);
  };
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-green-500">● {t('Online — EduSense AI')}</p>
            </div>
          </div>
              ...
              <AnimatePresence>
                {formatOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-1 w-44 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 z-50"
                  >
        ...
        <div className="flex-shrink-0 px-4 py-4 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 nav-safe">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 mb-3 scrollbar-hide">
            {['Explain this concept', 'Give me a quiz', 'Tell a story', 'Simplify this'].map(s => (
              <button
                ...
                className="px-3 py-1.5 rounded-full bg-primary/10 text-primary text-xs font-medium whitespace-nowrap hover:bg-primary/20 transition-all flex-shrink-0"
              >
                {t(s)}
              </button>
            ))}
          ...
          <div className="flex items-end gap-2">
            ...
            <div className="flex gap-1 flex-shrink-0">
              <label id="btn-attach-file" className="p-2.5 rounded-xl text-gray-400 hover:text-primary hover:bg-primary/10 cursor-pointer transition-all">
                <Paperclip size={20} />
            ...
            <div className="flex-1 relative">
              <textarea
                ...
                placeholder={t('Ask SAHAYAK anything...')}
                rows={1}
            ...
            <motion.button
              ...
              className="p-3 rounded-xl bg-primary text-white hover:bg-primary-700 disabled:opacity-40 transition-all flex-shrink-0"
